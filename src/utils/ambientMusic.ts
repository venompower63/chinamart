...
class AmbientMusicPlayer {
  ...
  private readonly droneFrequencies = [130.81, 196.00, 261.63]; // C3, G3, C4
   ...
   */
  private createSoftTone(frequency: number, duration: number, volume: number = 0.3): void {
  ...
  private playMelodyPattern(): void {
    ...
    for (let i = 0; i < patternLength; i++) {
      ...
      setTimeout(() => {
        this.createGuzhengPluck(notes[noteIndex] * (Math.random() > 0.7 ? 2 : 1));
      }, delay);
    }
  }
  ...
  private playChime(): void {
    ...
    osc.type = 'sine';
    osc.frequency.value = chimeFreqs[Math.floor(Math.random() * chimeFreqs.length)];
