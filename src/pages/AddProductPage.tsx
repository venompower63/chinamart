...
export default function AddProductPage() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		const newProduct = {
			...
			characteristics: formData.characteristics.reduce(
				(acc, char) => {
		...
		const storedProducts = JSON.parse(
			localStorage.getItem("chinamart_products") || "[]",
		);
		if (id) {
							...
							{formData.characteristics.map((char, idx) => (
								<div key={idx} className="characteristic-input">
									<input
										...
										value={char.key}
										onChange={(e) =>
											updateCharacteristic(idx, "key", e.target.value)
										}
									...
									<input
										...
										value={char.value}
										onChange={(e) =>
											updateCharacteristic(idx, "value", e.target.value)
										}
